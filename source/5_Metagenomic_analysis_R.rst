...
You can download the metadata of the dataset that we are analysing: `metadata_chimps.txt`_

  .. _metadata_chimps.txt: https://drive.google.com/file/d/1oedAI5qA_0RsVEL20oe8Yf80WmGnMzYR/view?usp=sharing
  
The metadata can be incorporated in your biom file as follows:
::

  # import mapping file with metadata
  biom_metadata <- import_qiime_sample_data("metadata_chimps.txt")
  # merge data
...
.. note::
  You may see that in some samples Kraken2 did not return any taxonomic classification. This is because the database is too small (2Gb!), and in some instance there may not enough resolution to make the taxonomic classification. 
...
::
  ...
  subsample = subset_samples(my_biom_rel_phylum, Group1=="Dental calculus chimp")
  # subsample multiple groups phyla
  subgroup = c("Dental_calculus_chimp","Dental_calculus","Oral_plaque")
  subsample = subset_samples(my_biom_rel_phylum, Group1 %in% subgroup)
